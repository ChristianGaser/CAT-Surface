...
void
composition_jacobian(int dm[], double *A, double *JA, double *B, double *JB,
                     double *C, double *JC)
{
...
void
composition_detjac(int dm[], double *A, double *dA, double *B, double *dB,
                   double *C, double *dC)
{
        double *Ax, *Ay;
        double *Bx, *By, jb;
...
double
initialise_objfun2(int dm[], double f[], double g[], double t0[],
...
double
initialise_objfun(int dm[], double f[], double g[], double t0[],
                  double J0[], double dj[], double b[], double A[])
{
        int j, m = dm[0] * dm[1];
        double ssl = 0.0, dt = 1.0;
        ...
        for (j = 0; j < m; j++) {
                ...
                d    = ((k11*dx1 + k21*dx2)*dy1 +
                        (k12*dx1 + k22*dx2)*dy2) - g[j];
                dx0  = ((k11     - k21    )*dy1 +
                        (k12     - k22    )*dy2);
                dy0  = ((k11*dx1 + k21*dx2)     -
                        (k12*dx1 + k22*dx2)    );
...
unwrap(int dm[], double f[])
{
        ...
        for (j = 0; j < dm[1]; j++) {
                pt = &f[j * dm[0]];
                if (j == 0) {
                        pt[0] -= floor(pt[0]/dm[0] + 0.5)*dm[0];
                } else {
                        pt[0] -= floor((pt[0] - pt[-dm[0]])/dm[0] + 0.5)*dm[0];
                }
                for (i = 1; i < dm[0]; i++) {
                        pt[i] -= floor((pt[i] - pt[i-1])/dm[0] + 0.5)*dm[0];
                }
        }
        for (i = 0; i < dm[0]; i++) {
                pt = &f[i + dm[0]*dm[1]];
                if (i == 0) {
                        pt[0] -= floor(pt[0]/dm[1] + 0.5)*dm[1];
                } else {
                        pt[0] -= floor((pt[0] - pt[-1])/dm[1] + 0.5)*dm[1];
                }
                for (j = dm[0]; j < dm[1]*dm[0]; j += dm[0]) {
                        pt[j] -= floor((pt[j] - pt[j-dm[0]])/dm[1] + 0.5)*dm[1];
                }
        }
}

int
dartel_scratchsize(int dm[], int issym)
{
        int m1, m2;
        int m = dm[0] * dm[1];
...
void
dartel(int dm[], int k, double v[], double g[], double f[], double dj[],
       int rtype, double param[], double lmreg, int cycles, int nits,
       int issym, double ov[], double ll[], double *buf)
{
        double *sbuf;
        double *b, *A, *b1, *A1;
        double *t0, *t1, *J0, *J1;
         ...
         *  [ A  A  A  t  t  J  J  J  J  t  t  J  J  J  J] for derivatives
         ...
         */
         /* cgs2(dm, A, b, rtype, param, 1e-8, 4000, sbuf, sbuf + 2*m,
          *      sbuf + 4*m, sbuf + 6*m); */
