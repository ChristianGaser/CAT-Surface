    ...
    if(J0!=(double *)0)
    {
        for(j=0; j<dm[1]; j++)
        {
            for(i=0; i<dm[0]; i++)
            {
                ...
                t0[o  ] = (i+1) + v[o  ]*td;
                t0[o+m] = (j+1) + v[o+m]*td;
                J0[o    ] = (v[bound(i+1,j  ,dm)  ]-v[bound(i-1,j  ,dm)  ])*td/2 + 1.0;
                J0[o+  m] = (v[bound(i+1,j  ,dm)+m]-v[bound(i-1,j  ,dm)+m])*td/2;
                J0[o+2*m] = (v[bound(i  ,j+1,dm)  ]-v[bound(i  ,j-1,dm)  ])*td/2;
                J0[o+3*m] = (v[bound(i  ,j+1,dm)+m]-v[bound(i  ,j-1,dm)+m])*td/2 + 1.0;
            }
        }
        for(i=0; i<k; i++)
        {
    ...
    if(J0!=(double *)0)
    {
        for(j=0; j<dm[1]; j++)
        {
            for(i=0; i<dm[0]; i++)
            {
                ...
                j00 = (v[bound(i+1,j  ,dm)  ]-v[bound(i-1,j  ,dm)  ])*td/2 + 1.0;
                j10 = (v[bound(i+1,j  ,dm)+m]-v[bound(i-1,j  ,dm)+m])*td/2;
                j01 = (v[bound(i  ,j+1,dm)  ]-v[bound(i  ,j-1,dm)  ])*td/2;
                j11 = (v[bound(i  ,j+1,dm)+m]-v[bound(i  ,j-1,dm)+m])*td/2 + 1.0;
