                     ...
                     int n_defects, int *holes, polygons_struct *hbw, polygons_struct *lbw)
{
    ...
    if (DEBUG)
        printf("create_polygon_point_neighbours...\n");
    create_polygon_point_neighbours(hbw, TRUE, &n_neighbours,
                                    &neighbours, NULL, NULL);

    if (DEBUG)
        printf("resample_defects_sph...\n");
    ...
    if (DEBUG)
        printf("compute_local_sharpness...\n");
    ...
    if (DEBUG)
        printf("patch_selfintersections...\n");
    ...
    smooth_iters = 50;
    if (DEBUG)
        printf("smooth_selfintersections with %d iterations...\n", smooth_iters);
    ...
    if (DEBUG)
        printf("compute_polygon_normals...\n");
                 ...
                 char *reparam_file, double max_refine_length, int force, double laplace_thresh)
{
    ...
    if (DEBUG)
        printf("find_topological_defects...\n");
    ...
    if (DEBUG)
        printf("get_equally_sampled_coords_holes...\n");
    ...
    if (DEBUG)
        printf("get_sph_coeffs_of_realdata...\n");
    ...
    *hbw_objects = create_object(POLYGONS);
    ...
    if (DEBUG)
        printf("get_realdata_from_sph_coeffs (lbw)...\n");
    ...
    *lbw_objects = create_object(POLYGONS);
    ...
    if (DEBUG)
        printf("sample_sphere_from_sph (lbw)...\n");
