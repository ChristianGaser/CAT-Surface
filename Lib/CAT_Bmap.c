          ...
          float *bias, int *dims, int pve, int verbose)
{
    ...
    if (verbose)
    {
        ...
        for (i = 0; i < n_classes; i++)
            fprintf(stdout, "%.3f*%.3f\t", mean[i], sqrt(var[i]));
        fprintf(stdout, "\n");
    }
    ...
    for (iters = 0; iters <= niters; iters++)
    {
            ...
            for (y = 1; y < dims[1] - 1; y++)
                for (x = 1; x < dims[0] - 1; x++)
                {
                    ...
                    if (psum > TINY)
                    {
                        ...
                        for (j = 0; j < n_classes; j++)
                            prob[(vol * j) + index] = 0.0;

                    // change the label
                    if (xi + BG != lab)
                        label[index] = (unsigned char)(xi + BG);
        ...
        if (verbose)
        {
            ...
            for (i = 0; i < n_classes; i++)
                printf("%.3f*%.3f ", mean[i], sqrt(0.5 / var[i]));
