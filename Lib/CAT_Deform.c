                 ...
                 double w[3], double sigma, float lim, int it, int verbose)
{
    ...
    double *displacement_tmp = malloc(sizeof(double) * polygons->n_points);
    ...
    for (v = 0; v < polygons->n_points; v++)
        displacement_tmp[v] = displacement_field[v][0];
    double threshold_x[2];
    get_prctile(displacement_tmp, polygons->n_points, threshold_x, prctile, 1, DT_FLOAT64);
    
    /* Get percentile for y-displacement */
    for (v = 0; v < polygons->n_points; v++)
        displacement_tmp[v] = displacement_field[v][1];
    double threshold_y[2];
    get_prctile(displacement_tmp, polygons->n_points, threshold_y, prctile, 1, DT_FLOAT64);

    /* Get percentile for z-displacement */
    for (v = 0; v < polygons->n_points; v++)
        displacement_tmp[v] = displacement_field[v][2];
    double threshold_z[2];
    get_prctile(displacement_tmp, polygons->n_points, threshold_z, prctile, 1, DT_FLOAT64);

    free(displacement);
    free(displacement_tmp);
    ...
    smooth_displacement_field(displacement_field, polygons, n_neighbours, neighbours, 5, 0.01*sigma);
