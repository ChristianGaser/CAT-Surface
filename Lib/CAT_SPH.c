 ...
 *
 * Reads SPH format file containing real and imaginary SH coefficients for 3D shape
 * (separate coefficient arrays for x, y, z coordinates). Allocates output arrays dynamically.
 * Bandwidth determines number of coefficients: bandwidth^2 elements per component.
 * Note: Implementation uses pointers-to-pointers for output allocation.
 *
 ...
 */
int read_SPHxyz(char *file, int bandwidth, double **rcx, double **rcy, double **rcz,
 ...
 *
 * Writes real and imaginary SH coefficients to SPH format file. Outputs all six
 * component arrays (rcx/icx for x, rcy/icy for y, rcz/icz for z) in interleaved
 * format per coefficient index. File header identifies format and dimensions.
 *
 ...
 */
int write_SPHxyz(char *file, int bandwidth, double *rcx, double *rcy, double *rcz,
                 double *icx, double *icy, double *icz)
{
    FILE *fp;
 ...
 *
 * Uses SH coefficients (x, y, z components) to compute coordinates at each vertex of
 * polygonal sphere mesh. Radial distance determined by (x^2+y^2+z^2)^(1/2) at each point;
 * constructs deformed mesh by mapping from reference sphere. Produces mesh-based representation
 * of shape encoded in SH coefficients.
 *
 ...
 *
 * For all SH degrees l < bandwidth_limited, copies coefficients from filtered
 * array (coeffs_filter) to output array (coeffs). Higher degrees remain unchanged.
 * Uses seanindex() to convert harmonic (m, l) indices to linear array index.
 *
 ...
 *
 * Calculates energy (power spectrum) at each spherical harmonic degree l as sum of
 * squared magnitudes of all (m,l) coefficients across x,y,z components. Output array
 * has bandwidth elements: shape_desc[l] = sum over m of (|rcx[m,l]|^2 + |icx[m,l]|^2 + ...).
 * Used for shape analysis and bandwidth selection.
 *
 * \param bandwidth (in) spherical harmonics bandwidth
 ...
 *
 * Multiplies SH coefficients by Butterworth filter kernel: 1/sqrt(1 + (l/bw_lim)^128).
 * Sharp cutoff around bandwidth_limited with steep rolloff. Used to smoothly suppress
 * high-frequency SH components while preserving low-frequency shape information.
 *
 * \param bandwidth         (in) total spherical harmonics bandwidth
 ...
 *
 * Zeros all SH coefficients outside frequency band [bw_lo, bw_hi]. Keeps coefficients
 * for degrees bw_lo <= l <= min(bw_hi, bandwidth-1) unchanged; zeros all others.
 * Sharp frequency cutoff (boxcar/rectangular window).
 *
 * \param bandwidth   (in) total spherical harmonics bandwidth
 ...
 *
 * Zeros all SH coefficients above degree bandwidth_limited. Keeps coefficients for
 * l < bandwidth_limited unchanged; discards higher frequencies (l >= bandwidth_limited).
 * Sharp frequency cutoff (rectangular/boxcar window).
 *
 * \param bandwidth         (in) total spherical harmonics bandwidth
 ...
 *
 * Transforms real-valued data on 2D sampled sphere (bandwidth x 2*bandwidth grid)
 * into SH coefficient arrays using Fast Fourier Transform (DCT/IFFT). Allocates
 * workspace for FFT computations; weights used for proper sphere geometry normalization.
 * Inverse operation to get_realdata_from_sph_coeffs().
 *
 ...
 *
 * Inverse transform: expands SH coefficient arrays back to real-valued spherical data
 * on 2D grid (bandwidth x 2*bandwidth) using inverse FFT (IDCT/IFFT). Allocates workspace
 * for IFFT computations. Inverse of get_sph_coeffs_of_realdata().
 *
 ...
 *
 * Maps 3D mesh vertices onto uniformly sampled 2D grid (bandwidth x 2*bandwidth points)
 * using spherical coordinates. Handles mesh holes and surface defects via interpolation.
 * Output coordinates represent mesh shape in format suitable for SH transform (get_sph_coeffs_of_realdata()).
 * Includes mesh scaling and boundary handling for proper projection.
 *
 ...
 *
 * Inverse of sub2ind: maps 1D flat array index i to 2D grid coordinates (x,y) using
 * modulo arithmetic. Handles periodic boundary conditions. Input parameters sy and sxy
 * are grid dimensions (sxy = sx*sy is total size; sy is height).
 *
 ...
 *
 * Computes discrete Laplacian (4-neighbor stencil) with periodic boundaries (longitude/latitude
 * wraparound). Handles masked regions (msk=0). Output array contains Laplacian values everywhere
 * non-masked. Used for smoothing and derivative computation on spherical grids.
 *
 ...
 *
 * Computes edge strength at each pixel using central differences: mag = sqrt(gx^2 + gy^2).
 * Handles periodic boundaries (longitude/latitude wraparound) for spherical coordinate grids.
 * Used for feature detection and edge preservation in 2D mesh projections.
 *
 * \param im        (in)  input floating-point 2D image
 ...
 *
 * Converts floating-point image to binary by comparison: pixels < threshold become 0,
 * pixels >= threshold become 1. Used for feature extraction, mask creation, and
 * boundary detection in 2D projected shape representations.
 *
 * \param im        (in)  input floating-point 2D image
 * \param dimx,dimy (in)  image dimensions (width x height)
