 ...
 *
 * Computes Voronoi areas, stable normals, mean curvature, and solves the
 * depth potential linear system using a cotangent Laplacian operator and
 * SOR Gauss-Seidel. Returns a per-vertex depth potential array.
 *
 ...
 *
 * Accumulates triangle contributions around each vertex using either
 * equal weights (lambda=1) or mixed Voronoi weights (lambda!=1).
 *
...
*/
/**
 * \brief Compute mean curvature using the cotangent Laplacian operator.
 *
 * Applies the Laplacian to vertex coordinates and projects the result
 * onto the vertex normals to estimate mean curvature.
 *
 * \param n_points (in)  number of vertices
 * \param coords   (in)  vertex coordinates
 * \param areas    (in)  per-vertex areas
 * \param normals  (in)  vertex normals
 * \param mat      (in)  cotangent Laplacian matrix in CSR format
 * \return Allocated per-vertex mean curvature array
 */
private double *
...
*/
/**
 * \brief Assemble the cotangent Laplacian operator in CSR format.
 *
 * Computes symmetric cotangent weights for each triangle and fills
 * the Laplacian matrix entries using the ordered neighbor lists.
 *
 * \param n_points (in)  number of vertices
 * \param coords   (in)  vertex coordinates
 ...
 *
 * Builds the right-hand side from mean curvature and areas, adds
 * regularization to the Laplacian, and solves using SOR Gauss-Seidel.
 *
 * \param n_points (in)  number of vertices
 * \param coords   (in)  vertex coordinates
 * \param areas    (in)  per-vertex areas
...
*/
/**
 * \brief Solve a CSR linear system with SOR Gauss-Seidel.
 *
 * Iteratively updates the solution using successive over-relaxation and
 * stops when the relative residual drops below the tolerance or the
 * maximum number of iterations is reached.
 *
 * \param nnode    (in)  number of rows
 * \param ia       (in)  CSR row pointers
 * \param ja       (in)  CSR column indices
 * \param mat      (in)  CSR coefficient values
...
*/
/**
 * \brief Initialize a CSR sparse matrix for the Laplacian operator.
 *
 * Allocates CSR arrays and fills the row structure based on vertex
 * neighbor lists, initializing all coefficients to zero.
 *
 * \param n_points (in)  number of vertices
 * \param n_ngh    (in)  neighbor counts per vertex
...
*/
/**
 * \brief Add a symmetric coefficient into a CSR matrix.
 *
 * Adds val to both (row,col) and (col,row) entries in the matrix.
 *
 * \param val (in)  coefficient value to add
...
*/
/**
 * \brief Compute stable vertex normals from triangle geometry.
 *
 * Computes weighted normals using edge-based weights that improve
 * stability on irregular meshes, then normalizes the result per vertex.
 *
 * \param n_points (in)  number of vertices
 * \param coords   (in)  vertex coordinates
