    ...
    for (z = 0; z < nz; z++)
    {
        for (y = 0; y < ny; y++)
        {
            for (x = 0; x < nx; x++)
            {
                ...
                if (x > 0 && x < nx - 1)
                    grad_x = (depth[i + 1] - depth[i - 1]) / (2.0 * voxelsize[0]);
                else if (x > 0)
                    grad_x = (depth[i] - depth[i - 1]) / voxelsize[0];
                else if (x < nx - 1)
                    grad_x = (depth[i + 1] - depth[i]) / voxelsize[0];

                if (y > 0 && y < ny - 1)
                    grad_y = (depth[i + nx] - depth[i - nx]) / (2.0 * voxelsize[1]);
                else if (y > 0)
                    grad_y = (depth[i] - depth[i - nx]) / voxelsize[1];
                else if (y < ny - 1)
                    grad_y = (depth[i + nx] - depth[i]) / voxelsize[1];

                if (z > 0 && z < nz - 1)
                    grad_z = (depth[i + nxy] - depth[i - nxy]) / (2.0 * voxelsize[2]);
                else if (z > 0)
                    grad_z = (depth[i] - depth[i - nxy]) / voxelsize[2];
                else if (z < nz - 1)
                    grad_z = (depth[i + nxy] - depth[i]) / voxelsize[2];
