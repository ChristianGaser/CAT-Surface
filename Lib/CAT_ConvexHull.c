...
surface_get_convex_hull(polygons_struct *polygons, polygons_struct *polygons_sphere)
{
    ...
    *object = create_object(POLYGONS);
    ...
    if (polygons_sphere != NULL)
    {
        ...
        object = resample_surface_to_target_sphere(convex_polygons, sphere_convex, polygons_sphere, NULL, NULL, 0, 0);
    }
...
/**
 * \brief Extract convex candidate points from a surface region.
 *
 * Uses vertex normals to test local support planes and keeps points that
 * are not exceeded by any neighbor, yielding a reduced set of candidates
 * for convex hull construction.
 *
 * \param polygons (in)  input surface mesh
 * \param points   (out) allocated array of candidate points
 * \return Number of candidate points
 */
private int
...
/**
 * \brief Compute clockwise angle in degrees for a 2D vector.
 *
 * Returns the clockwise angle in the range [0, 360) for vector (x,y),
 * with tolerance handling for near-axis cases.
 *
...
/**
 * \brief Find the limiting plane point for convex hull expansion.
 *
 * Scans candidate points to find the smallest clockwise angle around the
 * hinge direction while respecting the current supporting plane.
 *
 * \param n_points    (in)  number of candidate points
 * \param points      (in)  candidate points array
 * \param point_flags (in)  flags marking discarded or used points
 * \param centre      (in)  point on the plane
 * \param hinge       (in)  hinge direction vector
 * \param normal      (in)  plane normal vector
    ...
    Vector *normal)
{
    ...
    for_less(i, 0, n_points)
    {
        ...
        if (first || angle < best_angle)
        {
            ...
            {
                if (dbg)
                    printf("  found i = %d angle = %g side = %g\n", i, angle,
...
/**
 * \brief Map a source vertex index to a compact polygon vertex index.
 *
 * Adds the referenced point to the polygon point list if it has not
 * yet been added and returns the resulting index.
 *
 * \param polygons    (in/out) polygon mesh being constructed
 * \param points      (in)  source point array
 * \param new_indices (in/out) mapping from source to new index
 * \param v           (in)  source vertex index
 * \return New polygon vertex index
 */
private int
...
/**
 * \brief Append a polygon with given vertex indices to the mesh.
 *
 * Extends the polygon index list and end-indices array using the
 * provided vertex indices in order.
 *
 * \param polygons   (in/out) mesh to modify
 * \param n_vertices (in)  number of vertices in the polygon
 * \param vertices   (in)  vertex indices
 * \return Polygon index of the newly added polygon
 */
private int
add_polygon(
    polygons_struct *polygons,
    ...
    int vertices[])
{
    ...
    SET_ARRAY_SIZE(polygons->indices, n_indices, n_indices + n_vertices,
                   DEFAULT_CHUNK_SIZE);
    if (dbg2)
        printf("NEW POLY %d :", polygons->n_items - 1);
...
/**
 * \brief Compute a unique hash key for an undirected edge.
 *
 * Produces a symmetric key for the edge between two vertex indices,
 * independent of edge direction, for use in hash tables.
 *
 * \param polygons (in)  mesh containing the polygon
 * \param poly     (in)  polygon index
 * \param edge     (in)  edge index within polygon
 * \return Hash key for the edge
 */
private int
get_edge_key(
    polygons_struct *polygons,
...
/**
 * \brief Insert an edge into the processing queue and ref-count table.
 *
 * Tracks how many polygons reference each edge. Newly encountered edges
 * are queued for processing; repeated edges increment the ref-count.
 *
 * \param queue      (in/out) edge processing queue
 * \param edge_table (in/out) hash table of edge reference counts
...
/**
 * \brief Compute hull vertices lying on a supporting plane.
 *
 * Projects candidate points into a plane-aligned 2D coordinate system,
 * computes a 2D convex hull, and returns the ordered vertex indices for
 * the supporting plane of the current hull face.
 *
 * \param n_points    (in)  number of candidate points
 * \param points      (in/out) points array (may be adjusted onto plane)
 * \param point_flags (in/out) flags for used/discarded points
 * \param p0          (in)  first seed index
 * \param p1          (in)  second seed index
 * \param p2          (in)  third seed index
 * \param e0          (in)  optional edge seed index (or -1)
 * \param e1          (in)  optional edge seed index (or -1)
 * \param vertices    (out) hull vertex indices on the plane
 * \return Number of vertices on the plane hull
 */
private int
    ...
    int vertices[])
{
    ...
    for_less(i, 0, n_in_plane)
    {
        ...
        if (plane_points[i] == e0)
            i0 = i;
        if (plane_points[i] == e1)
            i1 = i;
    }
...
/**
 * \brief Build a 3D convex hull from a set of points.
 *
 * Constructs the convex hull mesh using a queue-driven expansion from an
 * initial face. Uses a hash table to track boundary edges and adds faces
 * until all edges are referenced twice.
 *
    ...
    polygons_struct *polygons)
{
    ...
    while (!IS_QUEUE_EMPTY(queue))
    {
        ...
        for_less(i, 0, size)
        {
            poly_vertices[i] = polygons->indices[POINT_INDEX(polygons->end_indices, poly, i)];
        ...
        if (dbg)
            printf("EDGE ENTRY %d:%d for poly %d\n", poly_vertices[edge],
...
/**
 * \brief Compute 2D convex hull indices using a gift-wrapping approach.
 *
 * Returns ordered indices of points on the 2D convex hull, optionally
 * constrained by a starting edge.
 *
 * \param n_points     (in)  number of 2D points
 * \param x            (in)  x coordinates
 * \param y            (in)  y coordinates
 * \param hull_indices (out) ordered hull indices
 * \param e0           (in)  optional start index (or -1)
 * \param e1           (in)  optional second start index (or -1)
 * \return Number of points on the 2D hull
 */
private int
get_convex_hull_2d(
    int n_points,
    ...
    int e1)
{
    ...
    do
    {
        ...
        for_less(i, 0, n_points)
        {
            ...
            {
                if (j == current_ind || j == i)
                    continue;
                cross = dx * (y[j] - y[current_ind]) - dy * (x[j] - x[current_ind]);
                if (cross < -TOLERANCE_DISTANCE)
                    break;
            }
            if (j == n_points)
            {
...
//
/**
 * \brief Extract point coordinates, normals, and neighborhood lists.
 *
 * Copies surface vertex coordinates and normals into new arrays and
 * constructs ordered neighbor lists for each vertex.
 *
 * \param surface       (in)  input surface mesh
 * \param n_points      (out) number of vertices
 * \param points        (out) allocated point array
 * \param normals       (out) allocated normal array
 * \param n_neighbours  (out) neighbor counts per vertex
 * \param neighbours    (out) neighbor index lists
...
//
/**
 * \brief Build ordered vertex neighbor lists for a triangular mesh.
 *
 * Constructs per-vertex neighbor lists by inspecting triangle indices,
 * then orders the neighbors into a closed loop around each vertex.
 *
 * \param surface              (in)  input triangular surface mesh
 * \param n_neighbours_return  (out) neighbor counts per vertex
 * \param neighbours_return    (out) ordered neighbor indices per vertex
 * \return OK on success, ERROR otherwise
 */
private int
