 ...
 */
void compute_points_centroid_and_normal_cg(polygons_struct *polygons,
 ...
 */
void get_polygon_vertex_curvatures_cg(polygons_struct *polygons, int n_neighbours[],
                                      ...
                                      int curvtype, double curvatures[])
{
    ...
    else
    {
        ...
        for (p = 0; p < polygons->n_items; p++)
        {
            ...
            {
                pidx = polygons->indices[POINT_INDEX(polygons->end_indices, p, vidx)];
