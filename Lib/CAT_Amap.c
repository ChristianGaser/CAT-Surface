...
/**
 * Compute a smooth, U-shaped weighting for a histogram bin using a sigmoid function.
 *
 * The weighting is maximal (peak) far from the "center" (bin intensity), and minimal (valley) at the center,
 * with a smooth transition determined by the "width" parameter. This can be used to emphasize
 * histogram bins corresponding to tissue borders (e.g., GM/WM) and suppress those in the core (e.g., GM).
 *
 * @param bin      The current histogram bin (typically 0–255).
 * @param center   The center of the "valley" (bin value of lowest weight).
 * @param width    The half-width of the valley (controls how broad the suppression is).
 * @param valley   The minimum weight (at the center).
 * @param peak     The maximum weight (at the sides).
 * @return         The weight to apply to the histogram bin.
 */
float u_weight(int bin, float center, float width, float valley, float peak)
{
    // Steepness factor: controls sharpness of transition (higher = steeper U).
    float s = 4.0f;
    float x = fabsf(bin - center) / width;
    
    // Sigmoid U-shaped weighting.
    float weight = valley + (peak - valley) / (1.0f + expf(-s * (x - 1.0f)));
    return weight;
}

/**
 * Perform 3D Contrast Limited Adaptive Histogram Equalization (CLAHE) on a float volume,
 * using a U-shaped, sigmoid-based weighting of histogram bins to enhance borders and suppress core intensities.
 * The operation is performed in-place, and the result overwrites the input volume.
 *
 * @param vol        Pointer to the 3D float volume (flattened array, overwritten with result).
 * @param dims[3]    Array of volume dimensions: dims[0]=X, dims[1]=Y, dims[2]=Z.
 * @param tile_x     Tile size in X (width).
 * @param tile_y     Tile size in Y (height).
 * @param tile_z     Tile size in Z (depth).
 * @param clip_limit Maximum allowed normalized count per histogram bin (limits contrast amplification).
 * @param valley     Minimum weight for histogram bins (at the U valley, i.e., intensity core).
 * @param peak       Maximum weight for histogram bins (at the U sides, i.e., borders).
 * @param center     Center of the U valley (bin value of lowest weight, e.g., ~GM mean).
 * @param width      Width parameter controlling the breadth of the U valley (in bin units).
 *
 * Example usage:
 *      clahe3d_float(vol, dims, 32, 32, 8, 2.0f, 0.2f, 1.0f, 132.0f, 30.0f);
 */
void clahe3d_float(
    float* vol, int dims[3],
    int tile_x, int tile_y, int tile_z,
    float clip_limit,
    float valley, float peak, float center, float width)
{
    int width_img = dims[0], height = dims[1], depth = dims[2];
    int num_bins = 256;
    int n_tiles_x = (width_img  + tile_x - 1) / tile_x;
    int n_tiles_y = (height + tile_y - 1) / tile_y;
    int n_tiles_z = (depth  + tile_z - 1) / tile_z;
    int i;

    // Allocate floating-point histogram and CDF arrays
    float ****hist = (float****)malloc(n_tiles_z * sizeof(float***));
    float ****cdf  = (float****)malloc(n_tiles_z * sizeof(float***));
    for (int tz = 0; tz < n_tiles_z; ++tz) {
        hist[tz] = (float***)malloc(n_tiles_y * sizeof(float**));
        cdf[tz]  = (float***)malloc(n_tiles_y * sizeof(float**));
        for (int ty = 0; ty < n_tiles_y; ++ty) {
            hist[tz][ty] = (float**)malloc(n_tiles_x * sizeof(float*));
            cdf[tz][ty]  = (float**)malloc(n_tiles_x * sizeof(float*));
            for (int tx = 0; tx < n_tiles_x; ++tx) {
                hist[tz][ty][tx] = (float*)calloc(num_bins, sizeof(float));
                cdf[tz][ty][tx]  = (float*)malloc(num_bins * sizeof(float));
            }
        }
    }
    ...
    for (i = 0; i < width_img*height*depth; ++i) {
        if (vol[i] < vmin) vmin = vol[i];
        if (vol[i] > vmax) vmax = vol[i];
    }
    if (vmax == vmin) vmax = vmin + 1.0f; // Prevent division by zero

    // Weighted histogram creation and clipping for each tile
    for (int tz = 0; tz < n_tiles_z; ++tz) {
        int z0 = tz * tile_z;
        int z1 = MIN(z0 + tile_z, depth);
        
        for (int ty = 0; ty < n_tiles_y; ++ty) {
            int y0 = ty * tile_y;
            int y1 = MIN(y0 + tile_y, height);
            
            for (int tx = 0; tx < n_tiles_x; ++tx) {
                int x0 = tx * tile_x;
                int x1 = MIN(x0 + tile_x, width_img);
                float* h = hist[tz][ty][tx];
                int n_voxels = (x1 - x0) * (y1 - y0) * (z1 - z0);
                float max_per_bin = clip_limit * n_voxels / num_bins;
                
                // Build weighted histogram
                for (int z = z0; z < z1; ++z)
                    for (int y = y0; y < y1; ++y)
                        for (int x = x0; x < x1; ++x) {
                            int idx = z*width_img*height + y*width_img + x;
                            float v = vol[idx];
                            int bin = (int)(255.0f * (v - vmin) / (vmax - vmin) + 0.5f);
                            bin = MAX(0, MIN(255, bin));
                            float weight = u_weight(bin, center, width, valley, peak);
                            h[bin] += weight;
                        }
                ...
                for (int b = 0; b < num_bins; ++b) {
                    if (h[b] > max_per_bin) {
                        excess += h[b] - max_per_bin;
                        h[b] = max_per_bin;
                    }
                }
                
                float bonus = excess / num_bins;
                for (int b = 0; b < num_bins; ++b) h[b] += bonus;
                for (int b = 0; b < ((int)excess % num_bins); ++b) h[b] += 1.0f;
            }
        }
    }
    ...
    for (int tx = 0; tx < n_tiles_x; ++tx) {
        float* h = hist[tz][ty][tx];
        float* c = cdf[tz][ty][tx];
        float sum = 0.0f;
        
        for (i = 0; i < num_bins; ++i) sum += h[i];
        float acc = 0.0f;
        
        for (i = 0; i < num_bins; ++i) {
            acc += (sum > 0.0f) ? (h[i] / sum) : 0.0f;
            c[i] = acc;
        }
    }

    // CLAHE output: in-place overwrite
    for (int z = 0; z < depth; ++z) {
        float fz = (float)z / tile_z - 0.5f;
        int tz0 = MAX((int)floor(fz), 0);
        int tz1 = MIN(tz0 + 1, n_tiles_z - 1);
        float wz = fz - tz0;
        
        for (int y = 0; y < height; ++y) {
            float fy = (float)y / tile_y - 0.5f;
            int ty0 = MAX((int)floor(fy), 0);
            int ty1 = MIN(ty0 + 1, n_tiles_y - 1);
            float wy = fy - ty0;
            
            for (int x = 0; x < width_img; ++x) {
                float fx = (float)x / tile_x - 0.5f;
                int tx0 = MAX((int)floor(fx), 0);
                int tx1 = MIN(tx0 + 1, n_tiles_x - 1);
                float wx = fx - tx0;
                int idx = z*width_img*height + y*width_img + x;
                int bin = (int)(255.0f * (vol[idx] - vmin) / (vmax - vmin) + 0.5f);
                bin = MAX(0, MIN(255, bin));
                
                // Trilinear interpolation of the CDFs
                float c000 = cdf[tz0][ty0][tx0][bin];
                float c100 = cdf[tz0][ty0][tx1][bin];
                float c010 = cdf[tz0][ty1][tx0][bin];
                float c110 = cdf[tz0][ty1][tx1][bin];
                float c001 = cdf[tz1][ty0][tx0][bin];
                float c101 = cdf[tz1][ty0][tx1][bin];
                float c011 = cdf[tz1][ty1][tx0][bin];
                float c111 = cdf[tz1][ty1][tx1][bin];
                ...
                float c = c0*(1-wz) + c1*wz;

                // Map back to original intensity range
                vol[idx] = vmin + c * (vmax - vmin);
            }
        }
    }
