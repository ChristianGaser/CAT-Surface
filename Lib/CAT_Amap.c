            ...
            for (x = 1; x < dims[0] - 1; x++)
            {
                ...
                if (beta != NULL)
                {
                    for (i = 1; i < n_classes; i++)
                    {
                        f[i - 1] = 0;
                        iBG = i + 1;
                        if ((int)label[z_area + y_dims + x - 1] == iBG)
                            f[i - 1]++;
                        if ((int)label[z_area + y_dims + x + 1] == iBG)
                            f[i - 1]++;
                        if ((int)label[z_area + ((y - 1) * dims[0]) + x] == iBG)
                            f[i - 1]++;
                        if ((int)label[z_area + ((y + 1) * dims[0]) + x] == iBG)
                            f[i - 1]++;
                        if ((int)label[((z - 1) * area) + y_dims + x] == iBG)
                            f[i - 1]++;
                        if ((int)label[((z + 1) * area) + y_dims + x] == iBG)
                            f[i - 1]++;
                    }
                        ...
                        for (i = 0; i < n_classes; i++)
                            for (j = 0; j < i; j++)
                            {
                                ...
                                if (color[i][f[0]][f[1]][f[2]][f[3]] < TH_COLOR ||
                                    color[j][f[0]][f[1]][f[2]][f[3]] < TH_COLOR)
                                    continue;
...
/**
 * \brief Local Potts prior for one voxel.
 *
 * Accumulates the Potts exponent over the 3x3x3 neighbourhood and turns it
 * into the prior probability of each label; every neighbour contributes with
 * the inverse of its physical distance.
 *
            ...
            for (k = -1; k < 2; k++)
                if (i != 0 || j != 0 || k != 0)
                {
                    ...
                    for (label1 = 1; label1 < n_classes + 1; label1++)
                    {
                        ...
                        if (class_weights)
                            class_weight = class_weights[label1 - 1];
                        if (class_weight < 0.0)
                            class_weight = 0.0;
                        if (label1 == label2)
                            similarity_value = same;
                        else if (abs(label1 - label2) < 2)
...
/**
 * \brief Iterated conditional modes for the Potts MRF prior.
 *
         ...
         double *voxelsize, int verbose, const double *class_weights)
{
    ...
    for (i = 0; i < 3; i++)
        sum_voxelsize += voxelsize[i];
    for (i = 0; i < 3; i++)
        voxelsize_squared[i] = SQR(3.0 * voxelsize[i] / sum_voxelsize);
                          ...
                          double *beta, int verbose, int use_median)
{
    ...
    for (iters = 0; iters < niters; iters++)
    {
        ...
        for (z = 1; z < dims[2] - 1; z++)
        {
            ...
            for (y = 1; y < dims[1] - 1; y++)
            {
                ...
                for (x = 1; x < dims[0] - 1; x++)
                {
                    ...
                    if (xBG + 1 != label_value)
                        label[index] = (unsigned char)(xBG + 1);
                }
            }
        }
        ...
        if (change_ll < TH_CHANGE)
            count_change++;
        if ((count_change > 2) && (iters > 5))
            break;
    ...
    if (verbose)
    {
        printf("\nFinal Mean*Std: ");
        for (i = 0; i < n_classes; i++)
            printf("%.3f*%.3f    ", mean[i], sqrt(var[i]));
        printf("\n");
    }
}
          ...
          int use_median, const double *mrf_class_weights, int use_multistep)
{
    ...
    for (i = 0; i < n_steps; i++)
    {
        ...
        if (step_sub_large < step_sub + 1)
            step_sub_large = step_sub + 1;
        ...
        if (verbose)
            printf("Amap sub-step %d/%d: sub=%d\n", i + 1, n_steps, step_sub);
    ...
    if ((niters_ICM > 0) && (weight_MRF > 0.0))
    {
        ...
        {
            for (i = 0; i < n_classes; i++)
                class_weights[i] = mrf_class_weights[i];
        }
