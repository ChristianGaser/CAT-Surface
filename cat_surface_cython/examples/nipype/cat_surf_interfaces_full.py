...
"""Reference (archived) Nipype interfaces for the full ``cat_surf`` API.

.. note::

   This is an **archived reference**, not a wired-up Nipype module.  The
   upstream ``nipype.interfaces.t1prep.cat_surf`` intentionally ships only the
   subset needed to replace FreeSurfer / FSL (bbreg) / ANTs in an fMRIPrep-style
   pipeline (denoising + volume/boundary registration).  The remaining
   surface/geometry/smoothing interfaces were moved here so they stay available
   for reuse without enlarging the Nipype PR surface or its test matrix.

   Imports are rewritten to absolute ``nipype.*`` paths and ``_import_cat_surf``
...
class CatSurfGetArea(SimpleInterface):
    ...
    Wraps ``cat_surf.get_area(vertices, faces) → (area, total_area)``, reading
    the surface from ``in_file`` and writing the per-vertex area to ``out_file``
    so the interface is self-contained (file in → file out) rather than passing
    numpy arrays between nodes.
    ...
    >>> node.inputs.in_file = 'lh.central.sub-01.gii'  # doctest: +SKIP
    >>> res = node.run()                    # doctest: +SKIP
    >>> res.outputs.out_file                # per-vertex area file  # doctest: +SKIP
    >>> res.outputs.total_area              # total surface area    # doctest: +SKIP
    """
    ...
    def _run_interface(self, runtime):
        cs = _import_cat_surf()
        # Default output name next to the working dir; respect an explicit
        # (possibly absolute) out_file when the user provides one.
        if isdefined(self.inputs.out_file):
            out_file = os.path.abspath(self.inputs.out_file)
        else:
            out_file = fname_presuffix(
                self.inputs.in_file, suffix="_area", newpath=runtime.cwd
            )
        v, fcs = cs.read_surface(self.inputs.in_file)
        area, total = cs.get_area(v, fcs)
        cs.write_values(out_file, area)
        self._results["out_file"] = out_file
        self._results["total_area"] = float(total)
        return runtime
...
class CatSurfSphericalDemonInputSpec(TraitedSpec):
    ...
    output_sphere_file = File(
        mandatory=True, desc="Output registered (warped source) sphere file path."
    )
    mask_file = File(
        exists=True,
        desc="Optional per-vertex cortex mask on the TEMPLATE mesh "
        "(0 = exclude, e.g. medial wall; >0 = include).",
    )
    n_steps = traits.Int(
        4,
        usedefault=True,
        desc="Number of multi-resolution pyramid levels, 1–4 (default 4).",
    )
    unfold = traits.Bool(
        False,
        usedefault=True,
        desc="Relax folded (negative-area) triangles in the final warp.",
    )
    verbose = traits.Bool(False, usedefault=True, desc="Print diagnostic output.")


class CatSurfSphericalDemonOutputSpec(TraitedSpec):
    output_sphere_file = File(desc="Registered (warped source) sphere file.")


class CatSurfSphericalDemon(SimpleInterface):
    """Perform diffeomorphic Spherical Demons surface registration.

    Wraps the file-based
    ``cat_surf.cli.surf_spherical_demon(source_file, source_sphere_file,
    ...
    input_spec = CatSurfSphericalDemonInputSpec
    output_spec = CatSurfSphericalDemonOutputSpec
    ...
    def _run_interface(self, runtime):
        ...
        kwargs = dict(
            ...
            output_sphere_file=out_file,
            n_steps=self.inputs.n_steps,
            unfold=self.inputs.unfold,
            verbose=self.inputs.verbose,
        )
        if isdefined(self.inputs.mask_file):
            kwargs["mask_file"] = self.inputs.mask_file
        cli.surf_spherical_demon(**kwargs)
        self._results["output_sphere_file"] = out_file
        return runtime
...
class CatSurfAverageInputSpec(TraitedSpec):
...
class CatSurfAverage(SimpleInterface):
    """Average two or more surface meshes vertex-wise.

    Wraps the file-based ``cat_surf.cli.surf_average(out_file, *in_files)``.
    ...
    def _run_interface(self, runtime):
        cli = _cat_surf_cli()
        out_file = os.path.abspath(self.inputs.out_file)
        cli.surf_average(out_file, *self.inputs.in_files)
        self._results["out_file"] = out_file
        return runtime
...
class CatSurfResampleToSphereInputSpec(TraitedSpec):
...
class CatSurfResampleToSphere(SimpleInterface):
    """Resample a surface mesh and optional per-vertex data to a target sphere.

    Wraps the file-based ``cat_surf.cli.surf_resample(source_surface_file,
    source_sphere_file, target_sphere_file, output_surface_file,
    input_values_file=…, output_values_file=…)`` (``CAT_SurfResample``).
    ...
    def _run_interface(self, runtime):
        cli = _cat_surf_cli()
        out_surface = os.path.abspath(self.inputs.output_surface_file)
        kwargs = {}
        ...
        if isdefined(self.inputs.output_values_file):
            out_values = os.path.abspath(self.inputs.output_values_file)
            kwargs["output_values_file"] = out_values
        cli.surf_resample(
            self.inputs.source_surface_file,
            self.inputs.source_sphere_file,
            self.inputs.target_sphere_file,
            out_surface,
            **kwargs,
        )
        self._results["output_surface_file"] = out_surface
        if out_values is not None:
            self._results["output_values_file"] = out_values
        return runtime
