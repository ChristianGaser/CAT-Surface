...
double fwhm  = 0.0;
double curvature_distance  = 3.0;
char *values_filename   = NULL;
char *surface_filename  = NULL;
int  sz_map[2] = {512, 256};

static ArgvInfo argTable[] = {
  {"-values", ARGV_STRING, (char *) 1, (char *) &values_filename, 
     "Optional file with values for mapping."},
  {"-surf", ARGV_STRING, (char *) 1, (char *) &surface_filename, 
     "Surface for mapping."},
  {"-distance", ARGV_FLOAT, (char *) 1, (char *) &curvature_distance,
...
Usage: %s -surf surface.obj|-values values_file output.pgm\n\n\
     ...
     Sheet image will be saved as binary PGM-file (default size 512 x 256).\n\n";
    ...
    char  *argv[] )
{
    ...
    if (ParseArgv(&argc, argv, argTable, 0) ||
      ((values_filename != NULL) && (surface_filename != NULL)) ||
      ((values_filename == NULL) && (surface_filename == NULL))) {
        fprintf(stderr,"You have to define either a surface or a value file.\n");
        usage( argv[0] );
    ...
    if( write_pgm(output_filename, data, sz_map[0], sz_map[1]) != 0 )
        return( 1 );
