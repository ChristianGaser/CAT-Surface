/* $Id: optimizersphere.c 964 2007-10-19 16:35:34Z john $ */
...
LtLf_le(int dm[], double f[], double s[], double g[])
{
        ...
        for (j = 0; j < dm[1]; j++) {
                ...
                for (i = 0; i < dm[0]; i++) {
                        ...
                        pgx[i] = wx0*px[0] + wy2*(px[jm1] + px[jp1]) +
                                 wx1*(px[im1] + px[ip1]) +
                                 wxy*(py[jp1 + im1] - py[jp1 + ip1] -
                                      py[jm1 + im1] + py[jm1 + ip1]);
                        pgy[i] = wy0*py[0] + wy1*(py[jm1] + py[jp1]) +
                                 wx2*(py[im1] + py[ip1]) +
                                 wxy*(px[jp1 + im1] - px[jp1 + ip1] -
                                      px[jm1 + im1] + px[jm1 + ip1]);
                }
...
#ifdef VERBOSE
        for (it = 0; it < 10 - ceil(1.44269504088896*log((double)dm[0])); it++)
                printf("  ");
...
LtLf_me(int dm[], double f[], double s[], double g[])
{
        ...
        for (j = 0; j < dm[1]; j++) {
                ...
                for (i = 0; i < dm[0]; i++) {
                        ...
                        pgx[i] = w00*px[0] + w01*(px[jm1] + px[jp1]) +
                                             w10*(px[im1] + px[ip1]);
                        pgy[i] = w00*py[0] + w01*(py[jm1] + py[jp1]) +
                                             w10*(py[im1] + py[ip1]);
                }
        }
}
        ...
        for (it = 0; it < 2*nit; it++) {
                ...
                for (j = 0; j < dm[1]; j++) {
                        ...
                        for (i = istart; i < dm[0]; i += 2) {
                                ...
                                sux = pbx[i] - (w01*(px[jm1] + px[jp1]) +
                                                w10*(px[im1] + px[ip1]));
                                suy = pby[i] - (w01*(py[jm1] + py[jp1]) +
                                                w10*(py[im1] + py[ip1]));
                                /*
                                 * syms axx ayy axy sux suy
                                 * A = [axx axy; axy ayy];
 ...
 */
void
cgs2(int dm[], double A[], double b[], int rtype, double param[], double tol,
     int nit, double x[], double r[], double p[], double Ap[])
{
        int i, it,  m = dm[0] * dm[1] * 2;
...
resize(int na[], double *a, int nc[], double *c, double *b)
{
        ...
        for (j = 0; j < nc[1]; j++) {
                ...
                o  *= na[0];
                cp = ap + na[0];
                for (ap = a, bp = b + j*na[0]; ap < cp; ap++, bp++)
        ...
        for (i = 0; i < nc[0]; i++) {
                ...
                wm  = wt2((o-1) - loc);
                cp = c + i;
                for (bp = b, ap = bp + na[0]*nc[1]; bp < ap; bp += na[0])
