...
main(int argc, char *argv[])
{
    ...
    opt.fwhm = &fwhm;
    opt.fwhm_surf = &fwhm_surf;
    opt.jacdet_file = jacdet_file;
    
    /* estimate rotation only */
    if (rotate) {
        if (CAT_SurfWarpSolveDartelFlow(src, src_sphere, trg, trg_sphere, prm, dm,
                        n_steps, rot, flow, -1, &opt) != OK)
            exit(EXIT_FAILURE);
    }
    ...
    for (run = 0; run < n_runs; run++) {
        if (CAT_SurfWarpSolveDartelFlow(src, src_sphere, trg, trg_sphere, prm, dm, n_steps,
                        rot, flow, loop, &opt) != OK)
            exit(EXIT_FAILURE);
        ...
        if (avg && (run==(n_runs-1))) {
            ...
            if (CAT_SurfWarpSolveDartelFlow(rsrc, rs_sph, rtrg, rt_sph, prm,
                                            dm, n_steps, rot, flow2, loop, &opt) != OK)
                exit(EXIT_FAILURE);
