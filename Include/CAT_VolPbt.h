...
extern "C"
{
    ...
    typedef struct
    {
        ...
        double sulcal_width;      /**< Max distance from CSF boundary (mm) for sulcal PPM correction (default: 5.0, 0=disable) */
        ...
        double barrier_local;     /**< FWHM in mm over which the gate follows regional thickness (default: 0, i.e. one global gate -- the regional form is measurably neutral on the three subjects it was tried on and is left off until it earns its place). Cortex is not one thickness: occipital runs near 2 mm while insular and temporal reach 3.5 mm, so a single gate calibrated on the whole hemisphere is simultaneously too high for the thin regions -- where two glued 2 mm banks imply only 4 mm and never reach it, which is why occipital sulci survive the correction -- and too low for the thick ones, where it catches ordinary tissue and leaves a visible seam in the thickness map. Smoothing the thickness proxy over this distance and gating against the local value fixes both: the gate follows the cortex it is standing in, and because it varies smoothly there is no boundary to see. The proxy is clipped at the global gate before smoothing, so the glued tissue the gate exists to find cannot inflate the very threshold meant to catch it. */
        double barrier_ramp;      /**< Width over which the gate fades in, as a fraction of the gate itself (default: 0.5; 0 restores a hard threshold). A hard threshold makes the correction switch on between one voxel and its neighbour, so on a thick cortex -- where a large part of the band sits near the gate -- capped and uncapped tissue end up side by side and the seam is visible in the thickness map as a step. Fading the cap in over a range removes the seam without changing what is corrected well above the gate or what is left alone well below it. */
        double barrier_gmtpct;    /**< Percentile below which the thickness proxy is averaged (default: 90). The proxy dist_WM + dist_CSF runs high because the glued sulci the gate exists to find sit in its upper tail; a median only limits their influence, it still sits inside a distribution they have skewed. Cutting the tail off and averaging what is left tracks the cortex more closely -- measured against the reported GMT on four hemispheres from two datasets, the ratio spans 0.087 for this estimator against 0.102 for the median, so a factor calibrated against it transfers better between subjects. Set 100 for a plain mean. */
        double barrier_gmtfactor; /**< Multiple of the reference thickness at which the gate sits (default: 1.5; <= 0 disables the gate). This is the criterion in its natural form: a glued sulcus is *two* cortices back to back, so the threshold belongs at twice the typical thickness of the brain being processed rather than at a fixed millimetre value that is only right for the cortex it was tuned on. The median is taken over dist_WM + dist_CSF inside the GM band, which for a band of locally constant thickness is that thickness exactly, and a median is unmoved by the glued minority it is meant to find -- which is the one assumption it makes: on a brain the glued fraction is small (1.6% of the cortex on the subject this was checked against) and the median is clean, but on data that is fused nearly everywhere the median measures the fusion itself and the derived gate comes out too loose. Pin barrier_gmtmax there. Note the median is of the *pre-projection* thickness proxy, which runs a little high against the GMT finally reported -- 2.72 mm against 2.36 mm on the subject this was checked on, about 15% -- so a factor of 2.0 here is nearer 2.3x the reported thickness. The value actually used is printed under verbose. Overridden by barrier_gmtmax when that is positive. */
        double barrier_gmtmax;    /**< Absolute override for the gate, in mm (default: 0 = derive it from barrier_gmtfactor and the data). This is the gate that matters. A glued sulcus does not merely look thick, it looks like *two* cortices back to back -- 5-6 mm where 2-3 mm is normal -- so the implied thickness at the voxel, dist_WM + dist_CSF, separates the two populations cleanly. Gating on the CSF distance alone (barrier_dmin) does not: plenty of ordinary voxels sit more than 2 mm from CSF simply by being near the white matter. Without this gate the collision test caps a fifth of the cortex on real data and the mean thickness collapses. */
        double barrier_gmtref;    /**< Reference thickness the gate is a multiple of, in mm (default: 0 = estimate it from this label map). Set it to a value derived beforehand with CAT_VolPbtBarrierReference() -- typically the mean over both hemispheres -- so that several label maps are gated by the same criterion. The estimate differs by up to 10% between the two hemispheres of a subject, mostly because they contain different amounts of fused sulci, and a hemisphere with more fusion gets both a looser gate and more correction; a shared reference made the corrected thickness more symmetric (mean |lh-rh| 0.120 -> 0.095 mm on the five most asymmetric of 19 subjects). barrier_gmtfactor still applies; barrier_gmtmax overrides both. */
        double barrier_dmin;      /**< Only bound a CSF distance that is already implausible for real cortex, in mm (default: 2.0). Without this the barrier fires wherever a collision is found, and since bounding the distance always shrinks GMT, the *mean* thickness of the whole brain becomes a readout of how often it fired -- which makes it move with barrier_q, exactly the parameter dependence the barrier was meant to remove. For a voxel in the middle of a 2.5-4 mm band dist_CSF is about 1.2-2 mm, whereas a front that ran across a glued sulcus returns 3 mm or more, so a threshold here separates the two and guarantees normally-thick cortex is never touched. Set 0 to disable the gate. */
     ...
     *
     * Runs the same preprocessing and distance estimation as
     * CAT_VolComputePbt() and returns the trimmed mean of dist_WM + dist_CSF
     * over the GM band (the values below opts->barrier_gmtpct) -- exactly the
     * reference a full run multiplies by barrier_gmtfactor, at a fraction of
     * the cost.  Derive it for several label maps, combine the values (e.g. the
     * mean over both hemispheres) and pass the result back through
     * opts->barrier_gmtref so that all of them are gated by the same
     * criterion.
     *
     * \param src       (in)  PVE label image (CSF=1, GM=2, WM=3), preprocessed
     *                        like the input of the PBT run it is meant for
     * \param dims      (in)  volume dimensions [nx, ny, nz]
     * \param voxelsize (in)  voxel sizes in mm [dx, dy, dz]
     * \param opts      (in)  PBT options; only the fields that shape the
     *                        distance maps (n_avgs, fast, range, pve_distance,
     *                        oriented_*) and barrier_gmtpct are used
     * \return reference thickness in mm, or a negative value on error or when
     *         the GM band is too small
     */
    double CAT_VolPbtBarrierReference(const float *src, int dims[3],
                                      double voxelsize[3],
