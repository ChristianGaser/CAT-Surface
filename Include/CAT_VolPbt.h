...
extern "C"
{
    ...
    typedef struct
    {
        ...
        int gyral_barrier;        /**< Stop the WM distance at the gyral medial surface (default: 0=off). The exact dual of sulcal_barrier, and the same routine finds it -- only the distance map fed to it changes. A thin white-matter blade lies between two sulci; where the classifier lost it the whole gyrus is grey matter, the fronts from the two pial surfaces run inward and collide at the midline the blade should have occupied, and that collision is the blade. Where the blade *was* segmented the white matter splits the band and no fronts ever meet, so this is an exact no-op there -- measured, not assumed: the collision count is 0. Bounding dist_WM by that surface raises the PPM at the gyral core, which is what strengthens the finger. */
