                                     ...
                                     struct TVertex vertices[])
{
  ...
  for (int r = tstart; r < tend; ++r) {
    ...
    int nbA = (v0==vid) ? v1 : (v1==vid ? v2 : v0);
    int nbB = (v0==vid) ? v2 : (v1==vid ? v0 : v1);
...
/* Return the two opposite vertices of triangles that contain edge (u,v).
   Writes up to two ids into opp[0..1]; returns how many were found (0..2, or >2 for non-manifold). */
static int edge_opposites(int u, int v, int opp[2],
                          struct TTriangle triangles[],
                          struct TRef refs[],
                          struct TVertex vertices[])
{
  ...
  for (int r = tstart; r < tend; ++r) {
    ...
    if ((a==u && b==v) || (a==v && b==u)) { if (found<2) opp[found++] = c; else ++found; }
    else if ((b==u && c==v) || (b==v && c==u)) { if (found<2) opp[found++] = a; else ++found; }
    else if ((c==u && a==v) || (c==v && a==u)) { if (found<2) opp[found++] = b; else ++found; }
  }
  return found;
}

/* Check Forstmann collapse (u<-v) preserves 2-manifold topology (link condition).
   - Disallow border collapses (keeps Euler characteristic).
   - Interior edge: N(u)∩N(v) must be exactly the two opposites of edge (u,v). */
static bool collapse_preserves_topology(int u, int v,
                                        struct TTriangle triangles[],
                                        struct TVertex vertices[],
                                        struct TRef refs[])
{
  /* Keep boundaries intact (constant χ and boundary component count). */
  if (vertices[u].border || vertices[v].border)
    return false;
  ...
  /* 1-ring neighbour sets (small valences; stack-local arrays suffice).
  ...
  for (int iteration = 0; iteration < max_iter; iteration ++) {
    ...
    loopi(0,ntriOK) {
      ...
      loopj(0,3)if(t->err[j]<threshold) {
        ...
        // Keep topology: disallow border collapses and enforce link condition
        if (v0->border || v1->border) continue;
        if (!collapse_preserves_topology(i0, i1, triangles, vertices, refs)) continue;
        
        // Compute vertex to collapse to
        vec3d p;
        calculate_error(i0,i1,&p, vertices);
        
        // Geometric safety: prevent flips
