...
#pragma omp parallel for collapse(2) private(i) reduction(+ : ss)
    for (j = 0; j < dm[1]; j++)
    {
        for (i = 0; i < dm[0]; i++)
        {
            ...
            tmp = (wx0 + paxx[ij]) * pux[ij] + paxy[ij] * puy[ij] + wy2 * (pux[bound(i, j - 1, dm)] + pux[bound(i, j + 1, dm)]) + wx1 * (pux[bound(i - 1, j, dm)] + pux[bound(i + 1, j, dm)]) + wxy * (puy[bound(i - 1, j - 1, dm)] - puy[bound(i - 1, j + 1, dm)] - puy[bound(i + 1, j - 1, dm)] + puy[bound(i + 1, j + 1, dm)]) - pbx[ij];
            ss += tmp * tmp;

            tmp = (wy0 + payy[ij]) * puy[ij] + paxy[ij] * pux[ij] + wy1 * (puy[bound(i, j - 1, dm)] + puy[bound(i, j + 1, dm)]) + wx2 * (puy[bound(i - 1, j, dm)] + puy[bound(i + 1, j, dm)]) + wxy * (pux[bound(i - 1, j - 1, dm)] - pux[bound(i - 1, j + 1, dm)] - pux[bound(i + 1, j - 1, dm)] + pux[bound(i + 1, j + 1, dm)]) - pby[ij];
            ss += tmp * tmp;
        }
    }
    return (ss);
...
void relax_le(int dm[], double a[], double b[], double s[], int nit, double u[])
{
    int it;
    double mu = s[2], lam = s[3], id = s[4];
    double wx0, wx1, wx2, wy0, wy1, wy2, wxy;
    ...
    regx = (4.0 * wxy - 2.0 * (wx1 + wy2)) - wx0;
    if (regx < 0)
        regx = 0.0;
    regy = (4.0 * wxy - 2.0 * (wx2 + wy1)) - wy0;
    if (regy < 0)
        regy = 0.0;
...
void relax_me(int dm[], double a[], double b[], double s[], int nit, double u[])
{
    int it;
    double w00, w01, w10;
    double *pux, *puy, *pbx, *pby, *paxx, *paxy, *payy;
...
void relax_be(int dm[], double a[], double b[], double s[], int nit, double u[])
{
    int it;
    double w00, w01, w02, w10, w11, w20;
    double lam;
...
void Atimesp_be(int dm[], double A[], double param[], double p[], double Ap[])
{
    int i, m = dm[0] * dm[1];
...
void addto(int n, double *a, double *b)
{
    int i;
    for (i = 0; i < n; i++)
        a[i] += b[i];
