...
#pragma omp parallel for schedule(static) private(jb)
    for (i = 0; i < mm; i++)
    {
        double x, y;
        double k11, k12, k21, k22;
            ...
            double J0[], double J1[])
{
    ...
    if (J0 != (double *)0)
    {
#pragma omp parallel for collapse(2) schedule(static) private(i)
        for (j = 0; j < dm[1]; j++)
        {
            for (i = 0; i < dm[0]; i++)
            {
...
#pragma omp parallel for collapse(2) schedule(static) private(j0)
    for (j1 = 0; j1 < dm[1]; j1++)
    {
        for (j0 = 0; j0 < dm[0]; j0++)
        {
...
#pragma omp parallel for schedule(static) reduction(+ : ssl) firstprivate(dt)
    for (j = 0; j < m; j++)
    {
        double x, y;
        int ix, iy;
...
#pragma omp parallel for schedule(static) reduction(+ : ssl)
    for (j = 0; j < m; j++)
    {
        double x, y;
        int ix, iy, k;
...
void dartel(struct dartel_prm prm, int dm[], double v[], double g[], double f[],
