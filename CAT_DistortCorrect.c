...
/* Program to make a conformal map more isometric using a distortion correction
 * algorithm. */
...
static ArgvInfo argTable[] = {
  ...
  {"-select", ARGV_CONSTANT, (char *) TRUE, (char *) &Select,
    "apply correction only if it lowers local area distortion" },
  {"-quiet", ARGV_CONSTANT, (char *) TRUE, (char *) &Quiet,
    "turn off progress reports" },
  {"-thresh", ARGV_FLOAT, (char *) 1, (char *) &Threshold,
       "apply correction only if local area distortion is above threshold."},
...
"\nUsage: %s [options] infile.obj conformalmap.obj outfile.obj\n\n\
    ...
    the distortion correction algorithm.  The original original mesh is\n\
    infile.obj and the conformal map of the mesh is conformalmap.obj.\n\
    Results are saved in outfile.obj.\n\n";
...
main(int argc, char** argv)
{
        ...
        double             **areas, localarea[128], centers[128];
        ...
        if (Quiet == 0)
                initialize_progress_report(&progress, FALSE, Iter,
                                           "DistortCorrect");
        ...
        for (p = 0; p < polygons->n_points; p++) {
                ...
                for (n = 0; n < n_neighbours[p]; n++) {
                        ...
                        if (areas[p][n] > 0) {
                                ...
                                totaldistort += localarea[n]/areas[p][n];
                        }
